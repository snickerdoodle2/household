<script lang="ts">
    ...
    function syncInternalValues(){
        type = {value: internal.type, label: internal.type === "gt" ? "Greater than" : "Lower than"};
        const initialSensor = sensors.find(sensor => sensor.id === internal.sensor_id);
        selectedSensor = {
...
<div class="flex flex-row gap-2 items-center whitespace-nowrap" bind:this={wrappingDiv}>
